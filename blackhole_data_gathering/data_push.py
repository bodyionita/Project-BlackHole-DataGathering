from blackhole_data_gathering.util import read_from_json_file, validate_number_of_years, get_start_end_date_touple
    ...
    def push_data(self):
        ...
        for s in symbols:
            if not os.path.exists(path + s['symbol'] + '.json'):
                ...
                self.push_symbol(s)
                self.available_symbols.append(s['symbol'])
        # TODO
        date_touple = get_start_end_date_touple(self.number_of_years)
        for date in range(date_touple[0], date_touple[1]):
            print(date)
